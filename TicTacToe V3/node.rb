...
class Node
    attr_reader :board, :value, :prev_move_pos

    def initialize(board ,value, prev_move_pos = nil)
        @board, @value, @prev_move_pos = board, value, prev_move_pos
    end

    def children
        children = []

        board.grids.each_with_index do |rows, i|
            rows.each_with_index do |val, j| 
                children << Node.new(board.dup, value == :x ? :o : :x, [i,j]) if val == '_'
                children.last.board[[i,j]] = value if val == '_'
            end
        end
    ...
    def win_node?(player)    
              #s/o won       winner is computer     if (tie or s/o won)
        return board.won? && board.winner == player if board.over?
        player == value ? children.any? { |child| child.win_node?(player) } : children.all? { |child| child.win_node?(player) }
    end

    def lose_node?(player)
        return board.won? && board.winner != player if board.over?
        player == value ? children.all? { |child| child.lose_node?(player) } : children.any? { |child| child.lose_node?(player) }
    end
end
